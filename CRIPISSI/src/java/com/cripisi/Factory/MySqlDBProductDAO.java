...
class MySqlDBProductDAO implements ProductDAO{
    ...
    @Override
    public ArrayList<Product> getSupplierProductUpdateList(Supplier so) {
       ResultSet rs = null;
        ArrayList<Product> searches = new ArrayList<>();
       PreparedStatement pstmst;
       Connection conn = MySqlDbDAOFactory.createConnection();
        try {
            pstmst = conn.prepareStatement(SQL_GET_SUPPLIER_UPDATELIST);
            pstmst.setInt(1, so.getSupplierTin());
             rs = pstmst.executeQuery();
             while(rs.next())
             {
                 Product two = new Product();
